...
import { registerStakeholderUser } from "../db/users";

export function createAuthRouter(deps: any) {
  const router = Router();
  const { 
    users, 
    getDbStatus, 
  ...
  router.post("/auth/register-stakeholder", auth(), async (req: any, res: any) => {
    ...
    let memUser = users.find((u: any) => u.id === uid || u.uid === uid || (phone && u.phone === phone) || (email && u.email === email));
    if (!memUser) {
      memUser = {
        id: uid,
        uid,
...
export function createMeRouter(deps: any) {
  const router = Router();
  const { auth } = deps;
