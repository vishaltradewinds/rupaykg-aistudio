      ...
      <section className="os-card rounded-2xl p-6">
        ...
        {green.map(g => (
          <div key={g.id} className="rounded-xl bg-white/5 border border-white/10 p-4">
            ...
            <div className="mt-4 text-xs text-white/60">Tradable: {String(g.tradable)} · Transferable: {String(g.transferable)}</div>
          </div>
        ))}
      </section>

      <section className="os-card rounded-2xl p-6">
        <div className="font-black text-white mb-3">CCTS methodologies</div>
        <div className="space-y-2">{methodologies.map(m => <div key={m.methodologyId} className="text-xs text-white/60">{m.methodologyId}</div>)}</div>
      </section>

      <div className="hidden"><ShieldCheck /><Building2 /><Tractor /><AlertTriangle /></div>
    </div>
  );
};
