...
export async function retireCredits(positionId: string, quantity: number, actorUid: string, idempotencyKey: string, reason: string) {
  ...
  try {
    ...
    const tx = await client.query(`INSERT INTO environmental_credit_transactions (id,position_id,transaction_type,quantity,idempotency_key,actor_uid,metadata) VALUES ($1,$2,'RETIRE',$3,$4,$5,$6) RETURNING *`, [crypto.randomUUID(),positionId,quantity,idempotencyKey,actorUid,JSON.stringify({reason})]);
    await client.query('COMMIT'); return tx.rows[0];
  } catch(e) { await client.query('ROLLBACK'); throw e; } finally { client.release(); }
}
