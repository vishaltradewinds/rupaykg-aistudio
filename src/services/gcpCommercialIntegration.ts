 ...
 *
 * The state machine is deliberately orchestration-only: every irreversible
 * state transition is backed by the corresponding custody or authoritative
 * registry service. No method here can manufacture issuance, transfer or
 * retirement evidence.
 */
export class GcpCommercialIntegration {
  ...
  async admitCustody(input: {
    ...
    const holding = await this.registry.verifyHolding(
      input.authoritativeCreditReference,
      input.holderEntityId,
    );

    if (holding.registry !== 'GCP_ICFRE' || holding.creditType !== 'GREEN_CREDIT') {
      throw new Error('GCP_AUTHORITY_BOUNDARY_VIOLATION');
    }
    if (holding.quantity !== input.quantity) {
      throw new Error('GCP_CUSTODY_QUANTITY_MISMATCH');
    }
    if (holding.registryAccountId !== input.registryAccountId) {
      throw new Error('GCP_REGISTRY_ACCOUNT_MISMATCH');
    }
  ...
  listTradableInventory() {
    return listAvailablePositions('GREEN_CREDIT');
  }
  ...
  async reserve(input: {
    ...
    const result = await reservePosition(
      input.positionId,
      input.quantity,
      input.actorUid,
  ...
  async transfer(input: {
    ...
    const eligible = await this.registry.verifyBuyerEligibility(
      input.buyerEntityId,
      'GREEN_CREDIT',
    );
    if (!eligible) throw new Error('GCP_BUYER_NOT_ELIGIBLE');

    const positionRows = await listAvailablePositions('GREEN_CREDIT');
    const position = positionRows.find((row: any) => row.id === input.positionId);
    if (!position) throw new Error('GCP_POSITION_NOT_FOUND_OR_NOT_LISTED');
    ...
    return {
      ...
      confirmation: { ...confirmation, custodyTransfer } as AuthoritativeTransferConfirmation,
    };
  }
  ...
  async reconcile(input: {
    ...
    if (holding.registry !== 'GCP_ICFRE' || holding.creditType !== 'GREEN_CREDIT') {
      throw new Error('GCP_RECONCILIATION_AUTHORITY_VIOLATION');
    }
    if (holding.quantity < input.expectedQuantity) {
      throw new Error('GCP_RECONCILIATION_QUANTITY_SHORTFALL');
    }
  ...
  async retire(input: {
    ...
    const positionRows = await listAvailablePositions('GREEN_CREDIT');
    const position = positionRows.find((row: any) => row.id === input.positionId);
    if (!position) throw new Error('GCP_POSITION_NOT_FOUND_FOR_RETIREMENT');
    ...
    if (confirmation.registry !== 'GCP_ICFRE' || confirmation.creditType !== 'GREEN_CREDIT') {
      throw new Error('GCP_RETIREMENT_AUTHORITY_VIOLATION');
    }
    if (!confirmation.authoritativeRetirementReference) {
      throw new Error('GCP_RETIREMENT_REFERENCE_REQUIRED');
    }
    ...
    const custodyRetirement = await retireCredits(
      ...
      input.idempotencyKey,
      input.reason,
      confirmation.authoritativeRetirementReference,
    );

    return { state: 'RETIRED' as const, confirmation, custodyRetirement };
  }

  static transition(from: GcpCommercialState, to: GcpCommercialState) {
    return advanceGcpCommercialState(from, to);
  }

  private assertTransferConfirmation(
    confirmation: AuthoritativeTransferConfirmation,
    input: { quantity: number; buyerEntityId: string },
  ) {
    if (confirmation.registry !== 'GCP_ICFRE' || confirmation.creditType !== 'GREEN_CREDIT') {
      throw new Error('GCP_TRANSFER_AUTHORITY_VIOLATION');
    }
