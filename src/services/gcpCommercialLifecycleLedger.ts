...
export async function getGcpCommercialState(positionId: string): Promise<GcpCommercialState | null> {
  const result = await pool.query(
    `SELECT state FROM gcp_commercial_lifecycle_events WHERE position_id=$1 ORDER BY sequence_no DESC LIMIT 1`,
    [positionId],
  );
  ...
  try {
    ...
    if (event.idempotencyKey) {
      const existing = await client.query(
        `SELECT state FROM gcp_commercial_lifecycle_events WHERE idempotency_key=$1`,
        [event.idempotencyKey],
      );
      if (existing.rows[0]) {
        await client.query('COMMIT');
        return existing.rows[0].state as GcpCommercialState;
      }
    }

    const latest = await client.query(
      `SELECT state, event_hash FROM gcp_commercial_lifecycle_events WHERE position_id=$1 ORDER BY sequence_no DESC LIMIT 1 FOR UPDATE`,
      [event.positionId],
    );
    const fromState = latest.rows[0]?.state as GcpCommercialState | undefined;
    if (fromState) {
      assertGcpCommercialTransition(fromState, event.toState);
    } else if (event.toState !== 'CUSTODY_ACTIVE') {
    ...
    await client.query(
      ...
      [
        ...
        event.authoritativeReference ?? null,
        event.quantity ?? null,
