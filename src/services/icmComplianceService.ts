...
export class ICMComplianceService {
  ...
  static suggestMethodology(activityType: string, context: "urban" | "rural" | string): { sector: string; methodologyId: string; status: "methodology_match" | "mrV_only" } {
    ...
    // Do not infer a landfill methodology from ordinary MSW collection/sorting.
    // A landfill/methane-specific activity must be present first.
    ...
    if (cleanContext === "rural" && /(livestock manure|animal waste|manure|biogas)/.test(activity)) {
