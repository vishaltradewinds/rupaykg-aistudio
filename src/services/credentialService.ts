...
export class CredentialService {
  ...
  public static issueCredential(
    subject: { id: string; claims: Record<string, any> },
    issuer: { id: string; name?: string } = { id: this.DEFAULT_ISSUER_DID, name: this.DEFAULT_ISSUER_NAME }
    ...
    const vcPayloadWithoutProof: Omit<W3CVerifiableCredential, 'proof'> = {
      ...
      issuer: issuer.id,
      issuanceDate,
      credentialSubject: { id: subject.id, ...subject.claims }
    };
  ...
  public static verifyCredential(vc: W3CVerifiableCredential, claimedIntegrityHash?: string): CredentialVerificationResult {
    ...
    try {
      ...
      return { isValid: false, proofStatus: 'FAILED', integrityDigestMatch, signatureVerified: false, guardianPolicyStatus: 'NOT_AVAILABLE', tampered: true, details: { credentialId, issuer, computedHash, claimedHash: claimedIntegrityHash, verificationError: `Verification exception: ${verErr.message}` }, message: `Cryptographic verification error: ${verErr.message}` };
    }
  }
}
